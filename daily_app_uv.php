...
$logname= "$logroot/visit_app_log/$year/visit_app_log_$stdate.gz";
$outprefix = "$logroot/daily_app_uv/$year/";
$auprefix = "$logroot/app_first_visit/$year/";
$chash = array();
...
$uv = array();
$au = array();
$outfd = popen("gzip -c >$outprefix/daily_app_uv_$stdate.gz","w");
if (!is_resource($outfd)) {
    glog("open pipe $outprefix/daily_app_uv_$stdate.gz failed");
    exit;
}
