...
CREATE TABLE IF NOT EXISTS `stat_lashou_trade` (
  ...
  KEY `ix_channel_id` (`channel_id`)
    ...
    $outfd = popen("gzip -c >$LOG_PATH/stat_lashou_trade/$logyear/stat_lashou_trade_$basedate_short.gz","w");

    foreach ($arr as &$part) {
        $sql = "replace into $table_stat_lashou_trade (stdate,trade_no,session_id,client_key,qdh_session,qdh_client,qdh_trade,visit_id,channel_id,
		city_id,buyer_id,goods_id,new_cat,type,sp_id,channel,price,cost_price,gross,amount,total_fee,buy_time) values ";
        $q = array();
        foreach ($part as &$r) {
            $channel_id = isset($r['channel_id']) ? $r['channel_id'] : "NULL";
            $str = sprintf("('%s','%s','%s','%s',%s,%s,%s,'%s',%s,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')",
                $basedate,$r['trade_no'],$r['session_id'],$r['client_key'],getqdhByStr($r['session_id']),getqdhByStr($r['client_key']),
		getqdhByStr($r['trade_no']),$r['visit_id'],$channel_id,$r['city_id'],$r['buyer_id'],$r['goods_id'],$r['new_cat'],
		$r['type'],$r['sp_id'],$r['channel'],$r['price'],$r['cost_price'],$r['gross'],$r['amount'],$r['total_fee'],$r['buy_time']
            );
            $q []= $str;
	    fprintf($outfd,"%s\n",str_replace(",","\t",str_replace(")","",str_replace("(","",str_replace("'","",$str)))));
        }
        $sql .= implode(',',$q);
        $dbhBA->exec($sql);
    }
