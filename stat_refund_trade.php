<?php
//此脚本生成bas_stat.stat_lashou_refund_trade同时在mfs系统记录日志，按年存储
...
// 写入数据表
$table_stat_lashou_refund_trade = 'bas_stat.stat_lashou_refund_trade';
$table_tmp = "stat_lashou_refund_trade_".date("Ymd",strtotime($basedate));
...
CREATE TABLE IF NOT EXISTS `stat_lashou_refund_trade` (
  ...
  `refund_type` smallint(6) DEFAULT NULL,
  `refund_reason` smallint(6) DEFAULT NULL,
  `action_type` smallint(6) DEFAULT NULL,
  `cps_type` smallint(6) DEFAULT NULL,
  PRIMARY KEY (`order_id`),
  KEY `ix_stdate` (`stdate`),
  KEY `ix_trade` (`trade_no`),
  KEY `ix_channel_id` (`channel_id`)
	...
	b.refund_type,b.refund_reason, b.action_type
	from $table_go_order b 
	inner join $table_trade_no v
	on b.trade_no = v.trade_no
	where b.add_time between '$basedate 00:00:00' and '$basedate 23:59:59' and (action_type=1 or action_type=2)";
    ...
    $sql = " delete from $table_stat_lashou_refund_trade where stdate = '$basedate'";
    $dbhBA->query($sql);
    ...
    $arr = array_chunk($tradeArr,500);
    foreach ($arr as &$part) {
        $sql = "replace into $table_stat_lashou_refund_trade (stdate,trade_no,channel_id,city_id,buyer_id,goods_id,new_cat,gross,amount,total_fee,pay_time,
		session_id,client_key,qdh_session,qdh_client,qdh_trade,visit_id,type,sp_id,channel,price,cost_price,order_id,convey_fee,
		payed,charge_pay,epurse_payed,youhui_fee,cps_type,refund_type,refund_reason,action_type) values ";
        $q = array();
        foreach ($part as &$r) {
		...
		$r['charge_pay'],$r['epurse_payed'],"NULL","NULL",$r['refund_type'],$r['refund_reason'],$r['action_type']
            );
            $q []= $str;
        }
        $sql .= implode(',',$q);
    ...
    system("mkdir -p $LOG_PATH/stat_lashou_refund_trade/$logyear");
    $sql = "select * from $table_stat_lashou_refund_trade where stdate='$basedate'";
    system("$MYSQL_BIN $sql_var -h".$gconf['MYSQL']['STAT']['HOST']." -u".$gconf['MYSQL']['STAT']['USER']." -p".$gconf['MYSQL']['STAT']['PASS']." -P".
                $gconf['MYSQL']['STAT']['PORT']." -Dbas_stat -e \"$sql\" |gzip -c >$LOG_PATH/stat_lashou_refund_trade/$logyear/stat_lashou_refund_trade_$basedate_short.gz");
